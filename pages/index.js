...
import loading from "../public/static/loading.json"
    ...
    // if(event.key === "Enter") {
      ...
      .then((response) => {
        ...
        if (response.data.weather[0].main == 'Rain') {
          ...
          setColor('#2698f0')
          setLoading(false)
        } else if (response.data.weather[0].main == 'Sunny' || 'Smoke') {
  ...
  return (
    <>
      ...
      { isLoading && 
        ...
        <p><Lottie animationData={loading} loop={true} /></p>
        </div>
      }
      {
      ...
      <div className={styles.weathercontent}>
       ...
       {
        !isLoading && weather && weather.map((w, index) => {
          return (
            // <div key={index}>
